...
**你必须直接输出纯 JSON，不要有任何额外的文字、解释、问候或确认。**
...
### 🔥 什么【必须跳过】的无意义镜头（Critical: Skip These）
以下类型的画面**不是有效镜头，必须完全忽略**：
- ❌ 全黑画面（Black screen）
- ❌ 全白画面（White screen）
- ❌ 纯色画面（Solid color screens）
- ❌ 转场效果（淡入淡出、叠化等过渡画面）
- ❌ 加载画面或缓冲画面
- ❌ Logo 闪屏或品牌标识单独画面
- ❌ 技术性画面（色条、测试卡等）

**重要**: 这些画面虽然在视频中存在，但**不应计入镜头总数**，也**不应出现在 shots 数组中**。
...
**🔥 最终检查清单（必须完成）:**
1. ✅ "shot_count" 等于你识别出的**有效镜头总数**（已排除全黑、全白等无意义画面）
2. ✅ "duration" 等于视频真实总时长
3. ✅ 所有 "duration_seconds" 之和等于 "duration"
4. ✅ 时间范围连续无间隙（0-2.5, 2.5-5.8, 5.8-9.0...）
5. ✅ **已排除所有无意义镜头**（全黑、全白、纯色、转场效果等）
6. ✅ 前3个**有效镜头**进行了像素级复刻
7. ✅ 每个镜头的 "description" 只描述第一帧静态画面
8. ✅ 所有视角和景别都从标准词汇表中选择
9. ✅ 所有情绪词汇都从标准词汇表中选择
10. ✅ "characters" 数组中使用完整格式（名称 + 特征）
11. ✅ 全局 "characters" 列表包含所有角色（去重）
