# VidFab 积分系统深度分析报告

**生成日期**: 2025-11-13  
**分析范围**: 积分数据模型、业务逻辑、API、用户注册流程  
**状态**: 完整分析  

---

## 目录

1. [核心发现](#核心发现)
2. [数据模型详解](#数据模型详解)
3. [积分业务逻辑](#积分业务逻辑)
4. [用户注册流程](#用户注册流程)
5. [现有功能分析](#现有功能分析)
6. [关键问题与建议](#关键问题与建议)

---

## 核心发现

### 积分系统概览

VidFab 已经实现了**完整、成熟的积分系统**，包括以下核心特性：

| 特性 | 现状 | 说明 |
|------|------|------|
| 积分与用户账户绑定 | ✅ 强绑定 | 积分存储在 `users` 表的 `credits_remaining` 字段 |
| 预分配积分 | ❌ 不支持 | 只有注册用户才能获得初始积分 |
| 批量操作 | ✅ 支持 | 存在 `batchCreditsOperation()` 方法 |
| 积分预扣 | ✅ 支持 | `reserve_user_credits()` 函数实现原子性预扣 |
| 交易历史追踪 | ✅ 完整 | `credits_transactions` 表记录所有交易 |
| 并发任务管理 | ✅ 完整 | 与积分预扣同时进行的并发限制 |

---

## 数据模型详解

### 1. users 表（核心表）

**文件**: `/Users/jacob/Desktop/vidfab/lib/database-schema.sql`

```sql
CREATE TABLE IF NOT EXISTS users (
    uuid UUID PRIMARY KEY,
    email VARCHAR(255) UNIQUE NOT NULL,
    -- ... 其他基础字段 ...
    
    -- 积分相关字段
    credits_remaining INTEGER DEFAULT 10,           -- 当前余额（新用户默认10）
    subscription_plan VARCHAR(20) DEFAULT 'basic',  -- free/lite/pro/premium
    subscription_status VARCHAR(20),                -- active/inactive/cancelled
    
    -- 扩展字段（来自 subscription-schema.sql）
    concurrent_jobs_running INTEGER DEFAULT 0,      -- 当前运行的并发任务数
    credits_last_reset_date TIMESTAMPTZ,            -- 上次月度重置的时间
    total_credits_earned INTEGER DEFAULT 0,         -- 历史总获得积分
    total_credits_spent INTEGER DEFAULT 0,          -- 历史总消费积分
);
```

**关键特性**:
- 积分直接存储在用户表，强制一一对应
- 支持三维跟踪：当前余额、历史收入、历史支出
- 没有"积分钱包"概念，没有预分配机制

---

### 2. credits_transactions 表（交易记录）

**文件**: `/Users/jacob/Desktop/vidfab/lib/database/subscription-schema.sql`

```sql
CREATE TABLE IF NOT EXISTS credits_transactions (
    id UUID PRIMARY KEY,
    user_uuid UUID NOT NULL REFERENCES users(uuid),
    
    -- 交易类型和数量
    transaction_type VARCHAR(20) NOT NULL, -- 'earned'|'spent'|'refunded'|'expired'|'bonus'
    credits_amount INTEGER NOT NULL,       -- 正数为收入，负数为消费
    balance_before INTEGER,                 -- 交易前余额
    balance_after INTEGER,                  -- 交易后余额
    
    -- 消费细节
    consumed_by VARCHAR(50),               -- 消费方式（如 'seedance-v1-pro-t2v'）
    video_job_id UUID,                     -- 关联的视频任务
    model_used VARCHAR(50),                -- 使用的模型
    
    -- 时间和元数据
    created_at TIMESTAMPTZ DEFAULT NOW(),
    metadata JSONB,
    description TEXT
);

-- 索引优化查询
CREATE INDEX idx_credits_transactions_user_uuid ON credits_transactions(user_uuid);
CREATE INDEX idx_credits_transactions_type ON credits_transactions(transaction_type);
CREATE INDEX idx_credits_transactions_created_at ON credits_transactions(created_at);
```

**重要特性**:
- 完整的审计日志（每笔交易都有记录）
- 支持多种交易类型：收入、消费、退款、过期、奖励
- 关联视频任务和模型信息，便于成本分析

---

### 3. credits_reservations 表（预扣记录）

**文件**: `/Users/jacob/Desktop/vidfab/lib/database/subscription-schema.sql`

```sql
CREATE TABLE IF NOT EXISTS credits_reservations (
    id UUID PRIMARY KEY,
    user_uuid UUID NOT NULL REFERENCES users(uuid),
    video_job_id UUID REFERENCES video_jobs(id),
    
    -- 预扣信息
    reserved_credits INTEGER NOT NULL,    -- 预扣数量
    model_name VARCHAR(50) NOT NULL,      -- 使用的模型
    estimated_cost INTEGER NOT NULL,      -- 预估成本
    
    -- 状态管理
    status VARCHAR(20) DEFAULT 'active',  -- 'active'|'consumed'|'released'|'expired'
    
    -- 时间管理
    created_at TIMESTAMPTZ DEFAULT NOW(),
    expires_at TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '10 minutes'), -- 10分钟超时
    consumed_at TIMESTAMPTZ,
    
    metadata JSONB
);
```

**设计意图**:
- 实现"预扣-消费-释放"三态模型，解决并发冲突
- 10分钟过期时间确保资源及时释放
- 支持任务失败时积分返还

---

### 4. 其他相关表

#### subscription_orders（订阅订单表）
```sql
CREATE TABLE IF NOT EXISTS subscription_orders (
    id UUID PRIMARY KEY,
    user_uuid UUID NOT NULL,
    order_type VARCHAR(20),    -- 'subscription'|'upgrade'|'downgrade'|'renewal'
    plan_id VARCHAR(20),       -- 'free'|'lite'|'pro'|'premium'
    credits_included INTEGER,  -- 随订阅包含的积分数量
    status VARCHAR(20),        -- 'pending'|'processing'|'completed'|'failed'
    stripe_subscription_id VARCHAR(255),
    -- ... Stripe 相关字段 ...
);
```

#### subscription_changes（套餐变更历史）
```sql
CREATE TABLE IF NOT EXISTS subscription_changes (
    id UUID PRIMARY KEY,
    user_uuid UUID NOT NULL,
    from_plan VARCHAR(20),
    to_plan VARCHAR(20),
    credits_before INTEGER,    -- 变更前的积分
    credits_after INTEGER,     -- 变更后的积分
    credits_adjustment INTEGER, -- 调整数量
    change_type VARCHAR(20),   -- 'upgrade'|'downgrade'|'new'|'cancel'|'renewal'
    effective_date TIMESTAMPTZ
);
```

---

## 积分业务逻辑

### 1. 核心服务类：CreditsManager

**文件**: `/Users/jacob/Desktop/vidfab/lib/subscription/credits-manager.ts` (602 行)

#### 主要方法

| 方法 | 功能 | 返回值 |
|------|------|--------|
| `reserveCredits()` | 预扣积分（任务开始） | 预扣ID |
| `consumeCredits()` | 消费预扣积分（任务完成） | 消费详情 |
| `releaseCredits()` | 释放积分（任务失败） | boolean |
| `checkCreditsAvailability()` | 检查余额是否足够 | CreditsBudgetInfo |
| `checkModelAccess()` | 检查模型访问权限 | ModelAccessCheck |
| `checkConcurrentJobs()` | 检查并发限制 | ConcurrentJobsCheck |
| `getCreditsHistory()` | 获取交易历史 | CreditsTransaction[] |
| `getActiveReservations()` | 获取活跃预扣记录 | CreditsReservation[] |
| `addBonusCredits()` | 手动添加积分（管理员） | 新余额 |
| `batchCreditsOperation()` | 批量处理积分操作 | boolean |

#### 代码示例：预扣流程

```typescript
async reserveCredits(
  userUuid: string,
  model: string,
  resolution: string,
  duration: string,
  videoJobId?: string,
  userEmail?: string
): Promise<string> {
  const requiredCredits = calculateCreditsRequired(model, resolution, duration);
  
  // 1. 查询用户当前余额和并发数
  let { data: user } = await supabaseAdmin
    .from(TABLES.USERS)
    .select('uuid, credits_remaining, concurrent_jobs_running, subscription_plan')
    .eq('uuid', userUuid)
    .single();
  
  // 2. 检查余额是否足够
  if (currentBalance < requiredCredits) {
    throw new Error(`Insufficient credits. Required: ${requiredCredits}, Available: ${currentBalance}`);
  }
  
  // 3. 检查并发限制（Free:1, 其他:4）
  const maxConcurrent = user.subscription_plan === 'free' ? 1 : 4;
  if (currentRunning >= maxConcurrent) {
    throw new Error(`Concurrent job limit exceeded`);
  }
  
  // 4. 调用RPC函数进行原子性操作
  const { data: reservationId } = await supabaseAdmin.rpc('reserve_user_credits', {
    p_user_uuid: userUuid,
    p_required_credits: requiredCredits,
    p_model_name: model,
    p_video_job_id: videoJobId,
    p_metadata: { resolution, duration, estimated_cost: requiredCredits }
  });
  
  // 5. 若RPC失败，降级使用直接更新
  if (!reservationId) {
    await supabaseAdmin.from(TABLES.USERS).update({
      credits_remaining: currentBalance - requiredCredits,
      concurrent_jobs_running: currentRunning + 1,
      updated_at: new Date().toISOString()
    }).eq('uuid', userUuid);
  }
  
  return reservationId;
}
```

### 2. 数据库函数：精细化原子性操作

**文件**: `/Users/jacob/Desktop/vidfab/lib/database/credits-functions.sql`

#### 关键 RPC 函数

##### ① reserve_user_credits（预扣）
```sql
CREATE OR REPLACE FUNCTION reserve_user_credits(
    p_user_uuid UUID,
    p_required_credits INTEGER,
    p_model_name VARCHAR(50),
    p_video_job_id UUID DEFAULT NULL,
    p_metadata JSONB DEFAULT '{}'
) RETURNS UUID AS $$
BEGIN
    -- 在单个事务中原子性完成：
    -- 1. 检查余额和并发数
    -- 2. 创建预扣记录
    -- 3. 更新用户余额和并发数
    
    INSERT INTO credits_reservations (...)
    VALUES (...) RETURNING id INTO reservation_id;
    
    UPDATE users
    SET credits_remaining = credits_remaining - p_required_credits,
        concurrent_jobs_running = concurrent_jobs_running + 1
    WHERE uuid = p_user_uuid;
    
    RETURN reservation_id;
END;
$$ LANGUAGE plpgsql;
```

**特点**: 在数据库层面保证原子性，避免并发冲突

##### ② consume_reserved_credits（消费）
```sql
CREATE OR REPLACE FUNCTION consume_reserved_credits(
    p_reservation_id UUID,
    p_actual_credits INTEGER
) RETURNS JSONB AS $$
BEGIN
    -- 获取预扣记录
    SELECT * INTO reservation_record
    FROM credits_reservations
    WHERE id = p_reservation_id AND status = 'active';
    
    -- 计算需要返还的积分
    refund_amount := reservation_record.reserved_credits - p_actual_credits;
    
    -- 更新用户余额（返还或额外扣除）
    UPDATE users
    SET credits_remaining = credits_remaining + refund_amount,
        total_credits_spent = total_credits_spent + p_actual_credits,
        concurrent_jobs_running = GREATEST(0, concurrent_jobs_running - 1)
    WHERE uuid = user_uuid;
    
    -- 记录消费交易
    INSERT INTO credits_transactions (...);
    
    -- 记录返还交易（如果有返还）
    IF refund_amount > 0 THEN
        INSERT INTO credits_transactions (...);
    END IF;
    
    RETURN jsonb_build_object('success', true, ...);
END;
$$ LANGUAGE plpgsql;
```

**特点**: 支持返还多余积分，确保精确消费

##### ③ batch_credits_update（批量更新）
```sql
CREATE OR REPLACE FUNCTION batch_credits_update(operations JSONB)
RETURNS BOOLEAN AS $$
DECLARE
    operation JSONB;
BEGIN
    -- 遍历所有操作
    FOR operation IN SELECT * FROM jsonb_array_elements(operations)
    LOOP
        -- 对每个用户进行积分更新
        UPDATE users
        SET credits_remaining = credits_remaining + credits_change,
            total_credits_spent = ...,
            total_credits_earned = ...
        WHERE uuid = user_uuid;
        
        -- 记录交易
        INSERT INTO credits_transactions (...);
    END LOOP;
    
    RETURN TRUE;
END;
$$ LANGUAGE plpgsql;
```

**特点**: 支持批量操作，每个操作都有完整的审计日志

---

## 用户注册流程

### 1. 注册入口

**文件**: `/Users/jacob/Desktop/vidfab/services/user.ts` (474 行)

#### saveUser() 函数流程

```typescript
export async function saveUser(userData: CreateUserData & { uuid?: string }): Promise<User> {
  const userUuid = userData.uuid || getUserUuidFromEmail(userData.email);
  
  // ✅ 关键修复：先检查用户是否已存在
  const { data: existingUser } = await supabaseAdmin
    .from(TABLES.USERS)
    .select('uuid, subscription_plan, credits_remaining, ...')
    .eq('uuid', userUuid)
    .single();
  
  let userToSave: Partial<DatabaseUser>;
  
  if (existingUser) {
    // === 已存在用户：只更新登录信息 ===
    console.log(`🔄 更新已存在用户: ${userUuid}`);
    userToSave = {
      uuid: userUuid,
      email: userData.email.toLowerCase().trim(),
      nickname: userData.nickname || userData.email.split('@')[0],
      // ... 其他字段 ...
      // ✅ 保留现有的订阅和积分信息（不覆盖）
      subscription_plan: existingUser.subscription_plan,
      subscription_status: existingUser.subscription_status,
      credits_remaining: existingUser.credits_remaining,
      total_videos_processed: existingUser.total_videos_processed,
    };
  } else {
    // === 新用户：初始化默认值 ===
    console.log(`✨ 创建新用户: ${userUuid}`);
    userToSave = {
      uuid: userUuid,
      email: userData.email.toLowerCase().trim(),
      // ... 其他字段 ...
      subscription_status: 'active',
      subscription_plan: 'free',
      credits_remaining: 50,           // ← 新用户初始积分：50
      total_videos_processed: 0,
      storage_used_mb: 0,
      max_storage_mb: 1024,
    };
  }
  
  // 使用 upsert 操作
  const { data, error } = await supabaseAdmin
    .from(TABLES.USERS)
    .upsert(userToSave, {
      onConflict: 'uuid',
      ignoreDuplicates: false
    })
    .select()
    .single();
  
  // ✅ 特殊处理：如果唯一性约束冲突，直接查询返回现有用户
  if (error?.code === '23505') {
    const { data: existingData } = await supabaseAdmin
      .from(TABLES.USERS)
      .select('*')
      .eq('uuid', userUuid)
      .single();
    return existingData;
  }
  
  return data;
}
```

### 2. 新用户初始积分分配

| 因素 | 当前值 | 备注 |
|------|-------|------|
| 初始积分 | **50** | 在 `saveUser()` 中硬编码 |
| 初始套餐 | **free** | 在 `saveUser()` 中硬编码 |
| 初始状态 | **active** | 在 `saveUser()` 中硬编码 |
| 分配时机 | **第一次注册** | 之后登录不会改变 |

**问题**: 新用户积分被硬编码为 50，无法灵活配置

### 3. 积分重置机制

**文件**: `/Users/jacob/Desktop/vidfab/lib/database/subscription-schema.sql` (行 189-231)

```sql
CREATE OR REPLACE FUNCTION reset_monthly_credits()
RETURNS void AS $$
DECLARE
    plan_credits INTEGER;
BEGIN
    -- 遍历所有需要重置的用户（上月未重置的用户）
    FOR user_record IN
        SELECT uuid, subscription_plan
        FROM users
        WHERE credits_last_reset_date < DATE_TRUNC('month', NOW())
    LOOP
        -- 根据套餐分配积分
        CASE user_record.subscription_plan
            WHEN 'free' THEN plan_credits := 50;
            WHEN 'lite' THEN plan_credits := 300;
            WHEN 'pro' THEN plan_credits := 1000;
            WHEN 'premium' THEN plan_credits := 2000;
            ELSE plan_credits := 50;
        END CASE;
        
        -- 更新用户积分
        UPDATE users
        SET credits_remaining = plan_credits,
            credits_last_reset_date = NOW(),
            total_credits_earned = total_credits_earned + plan_credits
        WHERE uuid = user_record.uuid;
        
        -- 记录交易
        INSERT INTO credits_transactions (...)
        VALUES (user_record.uuid, 'earned', plan_credits, ...);
    END LOOP;
END;
$$ LANGUAGE plpgsql;
```

**特点**: 
- 每个套餐有不同的月度积分配额
- 只有跨月时才会触发重置
- 完整的交易记录

---

## 现有功能分析

### 1. 批量操作功能

**✅ 已实现** `batchCreditsOperation()` 方法

```typescript
async batchCreditsOperation(
  operations: Array<{
    userUuid: string;
    creditsChange: number;
    transactionType: 'earned' | 'spent' | 'bonus' | 'refunded';
    description: string;
    metadata?: Record<string, any>;
  }>
): Promise<boolean> {
  try {
    await supabaseAdmin.rpc('batch_credits_update', { operations });
    return true;
  } catch (error) {
    console.error('Error in batch credits operation:', error);
    return false;
  }
}
```

**限制**: 只能操作已有用户，无法给未注册用户预分配

### 2. 管理员 API

**文件**: `/Users/jacob/Desktop/vidfab/app/api/admin/update-credits-direct/route.ts`

```typescript
export async function POST(req: NextRequest) {
  // 仅在开发环境可用
  if (process.env.NODE_ENV !== 'development') {
    return NextResponse.json({ success: false }, { status: 403 });
  }
  
  const { email, credits, plan } = await req.json();
  
  // 直接更新用户积分和套餐
  const { error } = await supabaseAdmin
    .from('users')
    .update({
      credits_remaining: credits,
      subscription_plan: plan,
      subscription_status: 'active'
    })
    .eq('email', email);
  
  return NextResponse.json({ success: !error, ... });
}
```

**特点**:
- ✅ 支持直接更新积分
- ❌ 仅在开发环境可用
- ❌ 没有审计日志

### 3. 积分检查 API

**文件**: `/Users/jacob/Desktop/vidfab/app/api/subscription/credits/check/route.ts`

支持以下检查：
1. **积分可用性检查** - `checkCreditsAvailability()`
2. **模型访问权限** - `checkModelAccess()`
3. **并发任务限制** - `checkConcurrentJobs()`

```typescript
POST /api/subscription/credits/check
{
  "model": "seedance-v1-pro-t2v",
  "resolution": "720p",
  "duration": "10s"
}

Response:
{
  "success": true,
  "can_afford": true,
  "current_balance": 300,
  "required_credits": 50,
  "remaining_jobs": 6,
  "warning_level": "none",
  "concurrent_info": {
    "current_running": 1,
    "max_allowed": 4,
    "can_start_new": true
  },
  "model_access": {
    "model": "seedance-v1-pro-t2v",
    "user_plan": "free",
    "can_access": true
  }
}
```

### 4. 用户积分查询 API

**文件**: `/Users/jacob/Desktop/vidfab/app/api/user/credits/route.ts`

```typescript
GET /api/user/credits

Response:
{
  "success": true,
  "data": {
    "credits": 300,
    "is_pro": false,
    "is_recharged": true,
    "plan_type": "free",
    "subscription": null
  }
}
```

---

## 关键问题与建议

### 问题 1: 积分与用户账户强绑定

**现状**: 积分存储在 `users.credits_remaining` 中，每个用户只有一个积分余额

**问题**:
- ❌ 无法给未注册用户预分配积分
- ❌ 无法实现"积分赠送"功能（只能更新余额，无法追踪来源）
- ❌ 无法支持多钱包概念（如"推荐奖励积分"与"购买积分"分别管理）

**建议**:
```typescript
// 方案A: 创建积分钱包表（推荐）
CREATE TABLE IF NOT EXISTS credit_wallets (
    id UUID PRIMARY KEY,
    user_uuid UUID REFERENCES users(uuid),  -- 允许NULL，支持未注册用户
    wallet_type VARCHAR(20),                -- 'free'|'purchased'|'bonus'|'referred'
    balance INTEGER DEFAULT 0,
    source VARCHAR(100),                    -- 来源说明
    created_at TIMESTAMPTZ,
    expires_at TIMESTAMPTZ,                 -- 有效期（可选）
    metadata JSONB
);

// 方案B: 预分配表（仅支持已注册用户）
CREATE TABLE IF NOT EXISTS credit_allocations (
    id UUID PRIMARY KEY,
    email VARCHAR(255),                     -- 邮箱，可用于未注册用户
    amount INTEGER NOT NULL,
    status VARCHAR(20) DEFAULT 'pending',   -- 'pending'|'allocated'|'expired'
    allocation_code VARCHAR(50),            -- 邑分码
    created_at TIMESTAMPTZ,
    allocated_at TIMESTAMPTZ,
    user_uuid UUID REFERENCES users(uuid)   -- 分配后填充
);
```

### 问题 2: 新用户积分初始化硬编码

**现状**:
```typescript
// services/user.ts 中硬编码
subscription_plan: 'free',
credits_remaining: 50,  // ← 硬编码
```

**问题**:
- ❌ 无法根据推荐渠道分配不同初始积分
- ❌ 无法支持促销活动（如"新用户送100积分"）

**建议**:
```typescript
// 创建初始化配置表
CREATE TABLE IF NOT EXISTS user_initialization_config (
    id UUID PRIMARY KEY,
    channel VARCHAR(50),            -- 'organic'|'referral'|'campaign'|'admin'
    initial_credits INTEGER,        -- 初始积分
    initial_plan VARCHAR(20),       -- 初始套餐
    expires_at TIMESTAMPTZ,         -- 活动有效期
    metadata JSONB
);

// 修改 saveUser 函数
export async function saveUser(userData: CreateUserData & { channel?: string }): Promise<User> {
    // 查询初始化配置
    const { data: config } = await supabaseAdmin
        .from('user_initialization_config')
        .select('initial_credits, initial_plan')
        .eq('channel', userData.channel || 'organic')
        .eq('expires_at', null, { foreignTable: null })
        .single();
    
    // 使用配置中的值，而不是硬编码的 50
    userToSave.credits_remaining = config?.initial_credits || 50;
}
```

### 问题 3: 管理员 API 缺乏生产环境支持

**现状**:
```typescript
if (process.env.NODE_ENV !== 'development') {
    return NextResponse.json({ success: false }, { status: 403 });
}
```

**问题**:
- ❌ 生产环境无法直接管理用户积分
- ❌ 大规模补偿用户时需要编写脚本

**建议**:
```typescript
// 创建带权限检查和审计日志的管理员 API
async function adminUpdateCredits(
    userEmails: string[],
    creditsAmount: number,
    reason: string,
    adminUuid: string // 执行者信息
): Promise<{ success: boolean; affected: number }> {
    // 1. 权限检查
    const adminUser = await checkAdminPermission(adminUuid);
    if (!adminUser) throw new Error('Unauthorized');
    
    // 2. 创建操作日志
    const operationId = generateId();
    await supabaseAdmin.from('admin_operations').insert({
        id: operationId,
        admin_uuid: adminUuid,
        operation_type: 'bulk_credit_update',
        affected_users: userEmails.length,
        details: { userEmails, creditsAmount, reason }
    });
    
    // 3. 执行批量更新
    await creditsManager.batchCreditsOperation(
        userEmails.map(email => ({
            userUuid: getUserUuidFromEmail(email),
            creditsChange: creditsAmount,
            transactionType: 'bonus',
            description: `${reason} (Operation: ${operationId})`
        }))
    );
}
```

### 问题 4: 预扣超时时间固定为 10 分钟

**现状**:
```sql
expires_at TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '10 minutes')
```

**问题**:
- ❌ 对于长时间处理的视频无法支持
- ❌ 无法根据模型/分辨率灵活设置

**建议**:
```typescript
const TIMEOUT_CONFIG = {
  'seedance-v1-pro-t2v': {
    '480p': '10 minutes',
    '720p': '15 minutes',
    '1080p': '20 minutes'
  },
  'veo3-fast': {
    '480p': '15 minutes',
    '720p': '20 minutes',
    '1080p': '30 minutes'
  }
};

async reserveCredits(model: string, resolution: string): Promise<string> {
    const timeout = TIMEOUT_CONFIG[model]?.[resolution] || '10 minutes';
    
    const { data: reservationId } = await supabaseAdmin.rpc(
        'reserve_user_credits',
        {
            p_expires_at: `NOW() + INTERVAL '${timeout}'`
        }
    );
}
```

### 问题 5: 交易类型值域有限

**现状**:
```sql
transaction_type VARCHAR(20) CHECK (transaction_type IN (
    'earned', 'spent', 'refunded', 'expired', 'bonus'
))
```

**问题**:
- 缺少"调整"、"转让"、"退款"等类型

**建议**:
```sql
-- 扩展交易类型
'earned'      -- 月度分配 / 购买
'spent'       -- 消费
'bonus'       -- 奖励（推荐、活动）
'refunded'    -- 退款（任务失败）
'expired'     -- 过期扣除
'adjusted'    -- 管理员调整
'transferred' -- 用户间转让
'disputed'    -- 争议处理
```

---

## 总结

### 优势

| 方面 | 评价 |
|------|------|
| 架构完整性 | ⭐⭐⭐⭐⭐ 完整的预扣-消费-释放流程 |
| 原子性保证 | ⭐⭐⭐⭐⭐ 使用数据库事务 RPC 函数 |
| 审计日志 | ⭐⭐⭐⭐⭐ 每笔交易都有记录 |
| 并发管理 | ⭐⭐⭐⭐⭐ 预扣表 + 任务计数器 |
| API 完善性 | ⭐⭐⭐⭐☆ 缺少生产环境管理工具 |

### 短板

| 短板 | 优先级 | 解决方案 |
|------|--------|--------|
| 不支持未注册用户预分配 | 高 | 创建预分配表或钱包表 |
| 新用户初始积分硬编码 | 中 | 配置化方案 |
| 管理 API 仅开发环境 | 高 | 增加权限检查 |
| 预扣超时固定 10 分钟 | 中 | 根据模型灵活设置 |

### 立即可用的功能

```typescript
// 给已注册用户批量增加积分
await creditsManager.batchCreditsOperation([
    {
        userUuid: 'user-1-uuid',
        creditsChange: 100,
        transactionType: 'bonus',
        description: '推荐奖励'
    },
    {
        userUuid: 'user-2-uuid',
        creditsChange: 50,
        transactionType: 'bonus',
        description: '活动奖励'
    }
]);

// 给单个用户添加积分
await creditsManager.addBonusCredits(
    userUuid,
    100,
    'Sign-up bonus',
    adminUuid
);

// 查看用户积分历史
const history = await creditsManager.getCreditsHistory(userUuid, 50, 0);

// 获取用户积分统计
const stats = await supabaseAdmin.rpc('get_user_credits_stats', {
    p_user_uuid: userUuid
});
```

---

## 推荐后续行动

### 第 1 步：扩展管理功能（1-2 天）
- [ ] 升级 `/api/admin/update-credits-direct` 支持生产环境
- [ ] 添加权限检查和审计日志
- [ ] 支持批量用户邮箱的积分更新

### 第 2 步：实现预分配机制（2-3 天）
- [ ] 创建 `credit_allocations` 表
- [ ] 实现分配码生成和验证逻辑
- [ ] 创建前端兑换页面

### 第 3 步：优化初始化配置（1-2 天）
- [ ] 创建 `user_initialization_config` 表
- [ ] 修改 `saveUser()` 支持动态配置
- [ ] 创建后台管理界面

### 第 4 步：增强灵活性（3-5 天）
- [ ] 扩展预扣超时配置
- [ ] 添加积分钱包概念（可选）
- [ ] 实现积分过期管理

---

**报告完成** | 版本: 1.0
