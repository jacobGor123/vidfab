...
export default function InputStage({ onStart }: InputStageProps) {
  ...
  const handleGetInspiration = async () => {
    ...
    try {
      const response = await fetch('/api/video-agent/inspirations')
      if (!response.ok) throw new Error('Failed to generate inspirations')
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-8">
      ...
      <div className="relative group">
        ...
        <Card className="relative bg-slate-950/50 border-2 border-dashed border-white/30 backdrop-blur-xl overflow-hidden shadow-[inset_0_2px_20px_rgba(0,0,0,0.5)] transition-all focus-within:border-blue-500/50 focus-within:bg-slate-900/80 focus-within:shadow-[0_0_0_2px_rgba(59,130,246,0.3)]">
          <div className="p-1">
            <Textarea
              value={script}
