...
// 新流程（2 步）- 2026-01-10 之后创建的项目
// 🆕 视频生成已集成到 Step 1，所以只需 2 步
const NEW_STEP_TITLES = {
  1: 'Script Analysis & Creation',  // 包含人物生成 + 分镜生成 + 视频生成
  2: 'Final Composition'            // 合成最终视频
}
...
export default function StepDialog({
  ...
  const totalSteps = isNewProject ? 2 : 5

  // 🔥 修复：新流程项目直接使用 step（1-2），旧项目才需要映射
  // 新流程：1 = Script Analysis + Videos, 2 = Final Composition
  ...
  const mapStepToNewFlow = (oldStep: number): number => {
    ...
    // 新流程项目：current_step 已经是 1-2 的格式，直接使用
    // 确保不超过 2
    return Math.max(1, Math.min(oldStep, 2))
  }
  ...
  const renderStep = () => {
    ...
    if (isNewProject) {
      switch (currentStep) {
        case 1:
          // Step 1: Script Analysis + Character Gen + Storyboard Gen + Video Gen
          return (
            <Step1ScriptAnalysis
              project={project}
              onNext={handleNext}
