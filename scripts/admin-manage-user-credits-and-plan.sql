...
DECLARE
    -- ⚙️ 请修改这里的配置
    v_target_email VARCHAR(255) := 'user@example.com';  -- 目标用户邮箱
    ...
    v_new_plan VARCHAR(20) := 'pro';  -- 新套餐类型: 'free', 'lite', 'pro', 'premium'
...
BEGIN
    ...
    IF v_credits_to_add != 0 THEN
        ...
        SET
            ...
            total_credits_earned = CASE
                WHEN v_credits_to_add > 0 THEN COALESCE(total_credits_earned, 0) + v_credits_to_add
                ELSE COALESCE(total_credits_earned, 0)
            END,
            total_credits_spent = CASE
                WHEN v_credits_to_add < 0 THEN COALESCE(total_credits_spent, 0) + ABS(v_credits_to_add)
    ...
    IF v_update_plan THEN
        ...
        -- 验证套餐类型
        IF v_new_plan NOT IN ('free', 'lite', 'pro', 'premium') THEN
            RAISE EXCEPTION '❌ 错误: 无效的套餐类型 "%". 有效值: free, lite, pro, premium', v_new_plan;
        END IF;
        ...
        INSERT INTO subscription_changes (
            ...
            v_new_plan,
            CASE
                WHEN v_old_plan IS NULL OR v_old_plan = 'free' THEN 'new_subscription'
                WHEN v_new_plan = 'free' THEN 'cancellation'
                ELSE 'upgrade'
            END,
            v_current_credits,
...
SELECT
    ...
    updated_at
FROM users
WHERE email = 'user@example.com'  -- ⚠️ 记得修改为你的目标邮箱
LIMIT 1;
...
WHERE u.email = 'user@example.com'  -- ⚠️ 记得修改为你的目标邮箱
...
JOIN users u ON sc.user_uuid = u.uuid
WHERE u.email = 'user@example.com'  -- ⚠️ 记得修改为你的目标邮箱
